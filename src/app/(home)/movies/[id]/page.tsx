      ...
      <div className="w-full max-w-4xl bg-gray-800 rounded-xl shadow-2xl p-6 sm:p-8 space-y-6 border border-gray-700">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
          ...
          <div className="text-left space-y-3">
            <p className="text-lg">
              <strong className="text-purple-300">Genre:</strong> {movie.Genre}
            </p>
            <p className="text-lg">
              <strong className="text-purple-300">Rated:</strong> {movie.Rated}
            </p>
            <p className="text-lg">
              <strong className="text-purple-300">Released:</strong>{" "}
              {movie.Released}
            </p>
            <p className="text-lg">
            ...
            <p className="text-lg">
              <strong className="text-purple-300">Writer:</strong>{" "}
              {movie.Writer}
            </p>
            <p className="text-lg">
              <strong className="text-purple-300">Actors:</strong>{" "}
              {movie.Actors}
            </p>
            <p className="text-lg">
              <strong className="text-purple-300">Language:</strong>{" "}
              {movie.Language}
            </p>
            <p className="text-lg">
              <strong className="text-purple-300">Country:</strong>{" "}
              {movie.Country}
            </p>
            <p className="text-lg">
              <strong className="text-purple-300">Awards:</strong>{" "}
              {movie.Awards}
            </p>
            <p className="text-lg">
              <strong className="text-purple-300">IMDb Rating:</strong>{" "}
              {movie.imdbRating} ({movie.imdbVotes} votes)
            </p>

            {movie.Plot && (
              <>
                <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-cyan-500 mt-6 mb-2">
                  Plot:
                </h2>
            ...
            {movie.Ratings && movie.Ratings.length > 0 && (
              <>
                ...
                <ul className="list-disc list-inside space-y-1 text-lg text-gray-200">
                  ...
                  {movie.Metascore !== "N/A" && (
                    <li>
                      <strong className="text-blue-300">Metascore:</strong>{" "}
                      {movie.Metascore}
                    </li>
                  )}
                </ul>
            ...
            <div className="mt-8 text-center md:text-left">
              <Link
                href="/"
                className="inline-block px-6 py-3 bg-gradient-to-r from-blue-600 to-teal-600 text-white font-semibold rounded-lg shadow-lg hover:from-blue-700 hover:to-teal-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-75 transition duration-200 transform hover:scale-105"
              >
                ← Back to Search
              </Link>
            </div>
